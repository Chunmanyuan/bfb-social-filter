...
from store.field_mapper import map_bilibili_to_media_item
...
class BilibiliCrawler(AbstractCrawler):
    ...
    async def search_by_keywords(self):
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while processed_videos_count < config.CRAWLER_MAX_NOTES_COUNT:
                if page < start_page:
                ...
                videos_res = await self.bili_client.search_video_by_keyword(
                    ...
                    order=SearchOrderType.LAST_PUBLISH,
                ...
                video_list = video_list[:config.CRAWLER_MAX_NOTES_COUNT - processed_videos_count]
                ...
                for video_item in video_items:
                    if video_item:
                        # Convert to unified MediaItem for SQLite
                        media_item = map_bilibili_to_media_item(video_item, task_id=getattr(config, 'TASK_ID', None))
                        
                        # Apply primary time filter
                        publish_time_s = int(video_item.get("View", {}).get("pubdate", 0))
                        ...
                        if filter_res == -1:
                            ...
                            utils.logger.info(f"[BilibiliCrawler.search_by_keywords] Video {video_item.get('View').get('aid')} is older than the start boundary. Triggering cutoff.")
                            ...
                            insert_media_item(media_item)
                            utils.logger.info(f"[BilibiliCrawler.search_by_keywords] Video {video_item.get('View').get('aid')} is newer than the end boundary. Skipping.")
                            continue
                ...
                if time_cutoff_triggered:
                    utils.logger.info("[BilibiliCrawler.search_by_keywords] Time cutoff triggered. Terminating search.")
                    break
                
                if processed_videos_count >= config.CRAWLER_MAX_NOTES_COUNT:
                    utils.logger.info(f"[BilibiliCrawler.search_by_keywords] Reached max videos count: {config.CRAWLER_MAX_NOTES_COUNT}")
                    break
    ...
    async def get_specified_videos(self, video_url_list: List[str]):
        ...
        for video_id in bvids_list:
            if video_id.lower().startswith('av'):
                aid_val = int(video_id[2:])
                task_list.append(self.get_video_info_task(aid=aid_val, bvid="", semaphore=semaphore))
            else:
    ...
    async def get_bilibili_video(self, video_item: Dict, semaphore: asyncio.Semaphore):
        ...
        # [New Feature] Pre-check video size via HEAD request before downloading
        try:
            import httpx
            async with httpx.AsyncClient(follow_redirects=True) as head_client:
                head_resp = await head_client.head(video_url, headers=self.bili_client.headers, timeout=10)
                content_length = head_resp.headers.get("content-length")
                if content_length and content_length.isdigit():
                    if not ItemFilter.check_video_size(int(content_length)):
                        utils.logger.warning(f"[BilibiliCrawler.get_bilibili_video] Video {aid} size exceeds limit, skipping download.")
                        return
        except Exception as e:
            utils.logger.warning(f"[BilibiliCrawler.get_bilibili_video] HEAD size check failed for {aid}: {e}, proceeding with download.")
        ...
        if task_id:
            ...
            local_path = f"data/bili/videos/{aid}/{extension_file_name}"
            update_media_downloaded("bilibili", str(aid), task_id, local_path)
